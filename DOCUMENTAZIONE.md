...
* `config.py` + `blog.config.json`: **Configurazione del blog (config-driven)**. Tutto ciò che è specifico del blog (brand, descrizione, categorie, priorità editoriali, parametri del brief, domini musicali, nome podcast) vive in `blog.config.json`; il codice lo legge da `config.py`. Per gestire più blog con lo stesso codice basta puntare la variabile d'ambiente `BLOG_CONFIG` a un file JSON diverso. `KANRI` è semplicemente il blog di riferimento (`blog.config.json`).
